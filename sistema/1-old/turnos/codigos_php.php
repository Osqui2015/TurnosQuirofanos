...
/* ----------- Hora inicio ------------------ */
	if(isset($_POST['mHoras'])){ 
		...
		/* /+ 10 minutos 
		$fechaAuxiliar	= strtotime ( "10 minutes" , strtotime ( $fechaEntrada ) ) ;	
		$horaFiPra	= date ( 'Y-m-d H:i' , $fechaAuxiliar ); */
		
		// echo 'FechaAuxiliar: ' . $fechaEntrada . '<br>';
		...
		for ($h = $HoIni; $h < 24 ; $h++){
			for ($m = 0; $m < 60 ; $m = $m + $Intervalo){
				...
				if ((($horaHoy >= $horaOcupada) && ($horaHoy <= $horaFinOcupada)) or ($mt == 1)){
						$z=1;
						$mt=0;
						if ($vec == $x){
							$x = $cnn;
					...
					if ($horaHoy > $horaFinOcupada){
						$op .= "<option value=".substr($hoy, 11).">".substr($hoy, 11)."</option>"; // mostrar horas disponibles
					}
					/* echo 'op'. strtotime($hoy)."<br>";
					echo substr($hoy, 11); */
				}
			}
