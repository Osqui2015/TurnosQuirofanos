...
if(isset($_POST['fbuscar'])){ 
    ...
    if(empty($quirodanoselec)){ //"No selecciono ningun quirofano"            
        ...
        <table style="width: 100%" class="table table-striped">
            <thead>
                <tr>
                    <th width="14%" scope="col">Quirofano</th>
...
if(isset($_POST['HoraFinTurno'])){ 
    ...
    $tiempoPracticar = $tiempoPracticar + $tiempomas;
...
if(isset($_POST['guardar'])){ 
    ...
    $fin = $fech.' '.$_POST['horaFin'].':00'; // HORA DE Fin
...
if(isset($_POST['misTurnos'])){
    ...
    $consulta = "SELECT tur.Numero, p.Nombre AS Medico, tur.Nombre AS Paciente, tip.Descripcion AS Tipo_Documento, DniPaciente, 
    ...
    Arco, ForceTriad, ForceTriad_Propio, Uti, Laparoscopica, ParaInternar, DescripcionCirugia, InsumosNoHabituales, Estado, usuario , DATE_FORMAT(FechaGrabacion,'%Y-%m-%d') AS fechaCreacion
    ...
    if (!empty($nombre)){
        $consulta.= "AND DATE_FORMAT(tur.FechaGrabacion,'%Y-%m-%d') = '$nombre' ";
    }
    $consulta.= "GROUP BY tur.Numero 
    ...
    $salida = '<div class="table-responsive">
        ...
        while($fila = $query->fetch_assoc()){                                                                              
                    $salida.= '<tr>
                        ...
                        <td>'.$fila['Quirofano'].'</td>
                        <td>'.utf8_encode($fila['Medico']).'</td>
                        ...
                        <td>'.$fila['HoraInicio'].'</td>
                        <td>'.$fila['HoraFin'].'</td>                
                        <td>'.$fila['CodigoPractica'].'</td>
                        <td>'.utf8_encode($fila['Descripcion']).'</td>
                        <td>'.$fila['Anestesista'].'</td>
                        <td>'.$fila['Rx'].'</td>
                        <td>'.$fila['Sangre'].'</td>
                        <td>'.$fila['Monitoreo'].'</td>
                        <td>'.$fila['Arco'].'</td>
                        <td>'.$fila['ForceTriad'].'</td>
                        <td>'.$fila['ForceTriad_Propio'].'</td>
                        <td>'.$fila['Uti'].'</td>
                        <td>'.$fila['Laparoscopica'].'</td>
                        <td>'.$fila['ParaInternar'].'</td>
                        <td>'.$fila['DescripcionCirugia'].'</td>
