...
/* -----------MUESTRA VALORES EN LA TABLA POR LA FECHA SELECCIONADA------------------ */
	if(isset($_POST['fbuscar'])){ 
		...
		$turnTabla = mysqli_query($conSanatorio,"SELECT tq.Numero,tq.Estado,
												...
												LEFT JOIN TurnosQuirofanosPracticas AS tu ON tu.Numero = tq.Numero
												LEFT JOIN PracticasQuirofano AS pa ON pa.Codigo = tu.Codigopractica
...
/* ----------- Hora inicio ------------------ */
	if(isset($_POST['mHorass'])){ 
		$mquir = $_POST['mquir'];
		$fechaSelec = $_POST['mfec'];

		echo 'fecha seleccionada mHoras'.$fechaSelec."<br>";
...
/* ---------------GUARDAR-------------- */
	if(isset($_POST['guardar'])){ 
		...
		$fech = $_POST['fechas']; // FECHA
			...
			$mifechaI = $fech.' '.$_POST['horaInicio'].':00'; // HORA DE Fin
		
		
			$NuevaFechaI = strtotime ($mifechaI);
		...
		$inicio =date('Y-m-d H:i:s',$NuevaFechaI);
			...
			$mifechaF = $fech.' '.$_POST['horaFinn'].':00'; // HORA DE Fin
		
		
			$NuevaFechaF = strtotime ($mifechaF);
...
/* ---------------GUARDAR MODIFICACIÓN-------------- */
	if(isset($_POST['guardarModi'])){ 
		...
		//FECHA SUSPENSIÓN
		//LOCAL O WEB
		$fechaGrabacion = date("Y-m-d"); //fECHA DE GRABACIÓN
		$arcoC = $_POST['arcoC']; //ARCO
		$ForceSana  = $_POST['ForceSana']; //ForceTriad Sanatorio
		$uti = $_POST['uti'];  //Uti
		$laparo = $_POST['laparo']; //LAPAROSCÓPICA		
...
if(isset($_POST['guardarTurnoModificar'])){ 
	...
	$resnum = mysqli_query($conSanatorio, "SELECT MAX(Numero+1) AS Numero FROM TurnosQuirofano");
		...
		$rest = substr($_POST['horaFinn'], 0, -1);
		
		$fin = $fech.' '.$rest.':00'; // HORA DE Fin
...
/* -----------CHEQUEAR MODIFICACIÓN------------------ */
if(isset($_POST['chequearModi'])){ 
	// $carlos = new DateTime('1982-08-15 15:23:48'); ejemplo
	$fech = $_POST['mfec'];
	$inicio = $fech.' '.$_POST['tiempoInicio'].':00';
	$fin = $fech.' '.$_POST['tiempoFin'].':00';
